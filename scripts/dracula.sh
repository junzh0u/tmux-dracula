...
main() {
  ...
  # Window option
  if $show_powerline; then
    tmux set-window-option -g window-status-current-format "#[fg=${window_sep_fg},bg=${window_sep_bg}]${window_sep}#[fg=${white},bg=${dark_purple}] #I #W #{pane_current_command} ${current_flags} #[fg=${dark_purple},bg=${bg_color}]${left_sep}"
  else
    tmux set-window-option -g window-status-current-format "#[fg=${white},bg=${dark_purple}] #I #W #{pane_current_command}${current_flags} "
  fi

  tmux set-window-option -g window-status-format "#[fg=${white}]#[bg=${bg_color}] #I #W #{pane_current_command}${flags}"
